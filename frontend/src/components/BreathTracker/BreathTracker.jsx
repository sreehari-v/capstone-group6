...
export default function BreathTracker({
  ...
  useEffect(() => {
    ...
    try {
      ...
      socketRef.current.on("connect", () =>
        console.debug("ws: connected", socketRef.current.id)
      );

      // Producer created a session
      socketRef.current.on("session_created", ({ code }) => {
        sessionCodeRef.current = code;
        isProducerRef.current = true;
        setStatus(`Sharing live data — code ${code}`);
      ...
      // Receive full snapshot (initial or incremental)
      socketRef.current.on("session_snapshot", (snapshot) => {
        if (!snapshot) return;
        ...
        if (snapshot.points && Array.isArray(snapshot.points)) {
          ...
          setPoints((ps) => {
            const next = ps.concat({
              x: snapshot.point.x || snapshot.t,
              y: snapshot.point.y || 0,
            });
            if (next.length > 400) next.shift();
            pointsRef.current = next;
            return next;
      ...
      // Producer asked to provide snapshot for a listener
      socketRef.current.on("request_snapshot", ({ to }) => {
        if (!to) return;
        const snapshot = {
          breathIn: breathInRef.current,
  ...
  const getRateColors = (bpmVal) => {
    ...
    if (bpmVal >= 12 && bpmVal <= 20)
      return { bg: "#d1fae5", color: "#065f46" }; // 12-20 Normal - Green
    if (bpmVal >= 21 && bpmVal <= 24)
      return { bg: "#fef3c7", color: "#92400e" }; // 21-24 Slightly elevated - Yellow
    if (bpmVal >= 25 && bpmVal <= 30)
      return { bg: "#fff4e6", color: "#92400e" }; // 25-30 High - Orange
    // >30 Danger - Red
  ...
  const start = async () => {
    ...
    const handler = (ev) => {
      ...
      const rawAxis =
        acc.y !== undefined && acc.y !== null
          ? acc.y
          : acc.x !== undefined && acc.x !== null
          ? acc.x
          : acc.z || 0;

      // estimate gravity (slow LP) and remove to get motion
      gravityRef.current =
        gravityAlpha * gravityRef.current + (1 - gravityAlpha) * rawAxis;
      const motion = rawAxis - gravityRef.current;

      // smooth motion signal
      const filtered =
        motionAlpha * lastFiltered.current + (1 - motionAlpha) * motion;
      ...
      emaMeanRef.current =
        emaAlpha * scaled + (1 - emaAlpha) * emaMeanRef.current;
      emaSqRef.current =
        emaAlpha * (scaled * scaled) + (1 - emaAlpha) * emaSqRef.current;
      const variance = Math.max(
        0,
        emaSqRef.current - emaMeanRef.current * emaMeanRef.current
      );
      ...
      if (!lastEventTime.current || t - lastEventTime.current > 120) {
        setPoints((ps) => {
          ...
          if (next.length > 400) next.shift();
          if (ps.length === 0)
            console.debug("BreathTracker: first plotted point", next[0]);
          return next;
        });
      ...
      if (
        scaled > adaptiveHyst &&
        lastDir.current !== 1 &&
        t - (listenerRef.current?.lastCountTime || 0) > minInterval
      ) {
        // inhale detected
        setBreathIn((v) => v + 1);
        ...
        while (
          pairedIndexRef.current < inhaleTimesRef.current.length &&
          pairedIndexRef.current < exhaleTimesRef.current.length
        ) {
    ...
    if (socketRef.current && isProducerRef.current && sessionCodeRef.current) {
      ...
      socketRef.current.emit("session_snapshot", {
        snapshot: { breathIn, breathOut, points, bpm },
        to: null,
      });
    }
    ...
    console.debug(
      "BreathTracker: setting startup timer for 2000ms waiting for devicemotion events"
    );
    startupTimerRef.current = setTimeout(() => {
      const msg =
        "No motion events received — device may not have motion sensors or permission was blocked.";
      setStatus(msg);
  ...
  useEffect(() => {
    breathInRef.current = breathIn;
  }, [breathIn]);
  useEffect(() => {
    breathOutRef.current = breathOut;
  }, [breathOut]);
  useEffect(() => {
          ...
          <div className="mt-4">
            {Chart ? (
              ...
              <div className="p-3 border rounded text-sm">
                ApexCharts not installed — install react-apexcharts & apexcharts
                for a realtime chart.
              </div>
            )}
          </div>
        </div>
